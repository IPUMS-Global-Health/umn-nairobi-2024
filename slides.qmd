...
subtitle: "Using R to understand climate and population health"
author:
...
-   IPUMS DHS standardizes and disseminates DHS survey data across time
    and space
:::

::: {.fragment .fade-in fragment-index="2"}
![](images/ipums_dhs.png){fig-align="center" height="400"}
:::
:::
...
-   Not very flexible...
    ...
    -   You can't redefine environmental exposure in the way most
...
argument, variable, assignment, pipe)
:::
...
## Loading DHS tabular data {auto-animate="true"}

```{r}
#| eval: false
# Load the metadata file
ke_ddi <- read_ipums_ddi("data/idhs_00022.xml")
```
...
#> Children who were not weighed are coded 9996."
```
:::

-   In this case, we will need to adjust the decimal places and remove
...
-   We can recode these large numbers as missing values (`NA`)
...
*TODO: Add additional vars that need cleaning here!*
...
-   This last point is particularly important when considering climate
    ...
    -   People who do not consistently live in an area will not have
        been exposed to the climate data for that area!

-   Many DHS surveys include an indication of residency status

    -   In IPUMS DHS: `RESIDEINTYR`
    -   In DHS: `V104`
...
::: nonincremental
::: {.fragment .fade-in fragment-index="1"}
-   It can also be useful to visualize variables to identify unreliable
    data
:::
...
-   Add different visual layers with `+`
...
-   It looks like lots of `BIRTHWT`values cluster at certain points

-   We should investigate to determine whether these values are reliable
...
::: notes
Contents? - I guess some dplyr stuff - Filter, select, mutate - Some
spatial calculations? - Distances...hm lots of things may need
projections - Intersections... - Spatial joins...this might be good
cause we have points + polygon sources

Should have some test activities for people to try to do? E.g. Could
some of these things be things people need to do some research on? -
E.g. just set aside 20 min and people can do some research to try to
find solutions to a few questions? - E.g. Find the top 10 counties with
an area greater than \_\_\_\_ - Find all counties that border a given
county, etc.
:::
...
## Mapping {auto-animate="true" visibility="uncounted"}
...
-   These are integrated boundaries, which can be obtained from [IPUMS
    DHS](https://www.idhsdata.org/idhs/gis.shtml) (TODO: probably should
    use year-specific!)

::: {.fragment .fade-in}
```{r}
ke_borders <- st_read("data/geo_ke1989_2014/geo_ke1989_2014_cleaned.shp") |> 
  filter(DHSCODE != 88) # Remove waterbodies
```
:::
...
-   Compared to vector data, raster data represent geographic features
    in a grid, where each cell in the grid is associated with a
    particular value.
...
-   Think "image" files. Like a camera: a rectangular set of pixels at a
    certain resolution
...
## Mapping with raster data {transition="fade"}

-   ggspatial works with raster data as well!

```{r}
#| echo: false
scale_chirts_c <- function(...) {
  colorRampPalette(c("#bad3e8", "#ffd3a3", "#da5831", "#872e38"), ...)
}
...
ggplot() +
  ...
  layer_spatial(ke_borders, fill = NA, color = "black") +
  # colorspace::scale_fill_continuous_sequential("Heat") +
  scale_fill_gradientn(colors = pal(256), na.value = NA)
```
:::
...
-   We need to *project* our spherical coordiantes onto a flat surface

::: {.fragment .fade-in}
![[Source](https://gistbok.ucgis.org/bok-topics/2017-quarter-03/map-projections)](images/map_proj.png){fig-align="center"
height="400"}
:::
...
::: notes
have people plot maps with different UTM coordinates to see how it
changes the projection visually
:::
...
-   How to summarize daily data?
    ...
    -   Sequential days above a given temperature ("heatwave")?
:::
:::

## Temporal aggregation in R {auto-animate="true"}
...
-   What about another way to aggregate? Days above a threshold
    temperature?
-   We can do simple operations to all of our raster cells at once:

::: {.fragment .fade-in}
```{r}
# Get a binary indicator of whether each cell is above 35 degrees
ke_bin <- ke_chirts > 35
```
:::
...
```{r}
#| eval: false
# Get a binary indicator of whether each cell is above
ggplot() +
  layer_spatial(ke_bin[[1]], alpha = 0.8) +
  layer_spatial(ke_borders, fill = NA, color = "#444444") +
  scale_fill_manual(values = c("#bad3e8", "#872e38"), na.value = NA)
```
...
ggplot() +
  layer_spatial(mask(ke_count_above_35, ke_borders), alpha = 0.9) +
  layer_spatial(ke_borders, fill = NA, color = "#444444") +
  scale_fill_gradientn(colors = pal(256), na.value = NA)
```
:::
...
...would be nice to demo a relative approach (e.g. Z-score) but hard
without longer sequence of data...

## What level do we want to aggregate to?

-   So far we have summarized the *entire* time series of CHIRTS data
    into one value
-   But the children in our DHS survey were born at specific points in
    time
-   A single summary value likely affects children differently based on
    when they were born

## What level do we want to aggregate to?

-   Instead we can aggregate to a lower level
-   DHS collects birth date data at the monthly level, so this is the
    finest resolution we can use

## What level do we want to aggregate to?

-   Recall that our raster has a `time` component
-   We can use it to aggregate our raster data to the monthly level as
    well
-   We will use terra's `tapp()` to do so
...
TODO: make this a nicer plot -- do panel stuff

```{r}
plot(ke_count_above_35_months)
```
...
```{r}
#| echo: false
ke_count_to_plot <- mask(ke_count_above_35_months[[1]], ke_borders)

ggplot() +
  layer_spatial(ke_count_to_plot, alpha = 0.9) +
  layer_spatial(ke_borders, fill = NA, color = "#444444") +
...
#| echo: false
ggplot() +
  layer_spatial(crop(ke_count_above_35_months[[1]], clust1, snap = "out")) +
  layer_spatial(clust1, fill = NA, color = "black") +
  scale_fill_gradientn(colors = pal(256), na.value = NA, limits = c(0, 1))
```
:::
...
::: {.fragment .fade-in}
```{r}
extract(ke_count_above_35_months[[1]], clust1)
```
:::
...
ke_chirts_clust <- extract(
  ke_count_above_35_months, # Extract values for each month of temperature data
...
::: notes
A heatwave definition/more complex temporal agg
:::
...
-   `extract()` produces data in **wide** format
    ...
    -   Each *column* corresponds to a month of temperature data for a
        given cluster
...
-   However, tabular data is often handled in **long** format
    ...
    -   Each *row* corresponds to a month of temperature data for a
        given cluster
...
# Get month names as used in extracted CHIRTS data
...
::: nonincremental
-   Worth noting that we can combine all the above steps into a single R
    expression.
-   I'm walking through step by step for clarity, but in a real analysis
    script, the following might be easier to write/interpret
:::
...
```{r}
# Each child is now associated with monthly proportion of hot days for their
# birth month
ke_dhs_chirts |>
  select(DHSID, KIDID, KIDDOBCMC, PROP_HOT_DAYS)
```
