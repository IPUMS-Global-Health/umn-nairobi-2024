...
-   You can't define environmental exposure in the way most
...
<!-- -   Another option: days above a threshold temperature -->

<!-- -   We can apply operations to all of our raster cells at once: -->
...
<!--   ) + -->
<!--   layer_spatial(ke_borders, fill = NA, color = "#4c4c4c", linewidth = 0.5) + -->
<!--   labs(title = "Areas above 35°C", subtitle = "January 1, 2013", fill = "") + -->
<!--   scale_fill_manual(values = c("#d6d6d6", "#872e38"), na.translate = FALSE) -->
<!-- ``` -->

<!-- ## 2. Count of hot days -->

<!-- -   Now we can sum to count the number of days above 35 degrees -->
...
<!--   labs(title = "Number of days above 35°C", subtitle = "Kenya, 2013", fill = "") + -->
<!--   scale_fill_chirts_c() -->
<!-- ``` -->

## Monthly aggregation

-   This approach produces an average for the *entire* time series of CHIRTS 
    data

-   The children in our DHS survey were born at *specific* points in time

-   This single average does not reflect the conditions that each
...
-   We can summarize our temperature data to the *monthly* level
    ...
    -   Then we can isolate the temperature conditions **around each child's
        birth date**
...
-   What is considered "hot" in one area might not be in another area with
    a different baseline climate
...
-   To speed things up, we have already created rasters containing the 10-year
...
-   We still have a raster, but the values now reference *differences* between
    2013 temperature and a "normal" temperature
...
-   For the rest of our demo, we'll use *average monthly temperature* to keep
    things simple
