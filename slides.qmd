...
author:
  ...
  # - name: Finn Roberts
  #   affiliations: IPUMS Senior Data Analyst
  # - name: Kathryn Grace
  #   affiliations: UMN Department of Geography
format:
  revealjs:
...
-   For `EDUCLVL`, we may want to combine the "Secondary" and "Higher" groups:

::: {.fragment .fade-in}
```{r}
ipums_val_labels(ke_dhs$EDUCLVL)
```
:::
...
-   Fortunately, when we use dplyr, we don't have to indicate the name of the
    dataset each time:
...
ggplot() +
  ...
  labs(title = "DHS Cluster Locations", subtitle = "Kenya 2014 Sample")
```
:::
...
:::{.notes}
Source: https://gistbok.ucgis.org/bok-topics/2017-quarter-03/map-projections
:::
...
ggplot() +
  ...
  layer_spatial(ke_borders, fill = NA, color = "#4c4c4c", linewidth = 0.5) +
  labs(title = "Mean Temperature (°C)", subtitle = "Kenya, 2014", fill = "") +
  scale_fill_chirts_c()
```
:::
...
ggplot() +
  layer_spatial(
    mask(ke_bin[[1]], ke_borders), 
    alpha = 0.9
  ) +
  layer_spatial(ke_borders, fill = NA, color = "#4c4c4c", linewidth = 0.5) +
  labs(title = "Areas above 35°C", subtitle = "January 1, 2014", fill = "") +
  scale_fill_manual(values = c("#d6d6d6", "#872e38"), na.translate = FALSE)
```
...
ggplot() +
  ...
  layer_spatial(ke_borders, fill = NA, color = "#444444") +
  labs(title = "Number of days above 35°C", subtitle = "Kenya, 2014", fill = "") +
  scale_fill_chirts_c()
```
...
-   For simplicity of interpretation in our models (later), we'll return to the
...
ke_chirts_mean_mnths <- tapp(
  ke_chirts, # No longer the binary raster!
  fun = mean,
  index = "yearmonths"
)
```
...
#| echo: false
#| fig-align: center
ke_count_to_plot <- mask(ke_chirts_mean_mnths[[1]], ke_borders)

ggplot() +
  layer_spatial(ke_count_to_plot, alpha = 0.9) +
  layer_spatial(ke_borders, fill = NA, color = "#444444") +
  layer_spatial(ke_buffer, fill = NA, color = alpha("black", 0.5)) +
  labs(title = "Mean Temperature (°C)", subtitle = "January, 2014", fill = "") +
  scale_fill_chirts_c()
```
:::
